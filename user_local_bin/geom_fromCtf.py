...
# --------------- read data -----------------------------------------------------------------------
  ...
  while table.data_read():
    words = table.data
    if not annotationSkip:
      if containletter(words) or words[0].isspace():                                                                               # process initial comments/header block
        continue
      else:
        annotationSkip = True
...
# --------------- determine size and grid ---------------------------------------------------------
  ...
  if any([np.any(eulerangles[:,i]>=limits[i]) for i in [0,1,2]]):
    errors.append('Error: euler angles out of bound. Ctf file might contain unidexed poins.')
