 ...
 do while (trim(line) /= IO_EOF)                                                                    ! read through sections of material part
   ...
   if (section > 0_pInt) then
     ...
     select case(tag)
       ...
       case ('ebsd_ngrains')
         EBSD_Ngrains = IO_intValue(line,chunkPos,2_pInt)
...
if (EBSD_Ngrains == 0_pInt ) then ! added by zhm
 echo = IO_globalTagInPart(fileUnit,myPart,'/echo/')

 Nsections = IO_countSections(fileUnit,myPart)
else
 echo = .false.
 Nsections = EBSD_Ngrains
endif
...
if (EBSD_Ngrains == 0_pInt ) then ! added by zhm
 ...
 microstructure_Nconstituents = 1_pInt
 microstructure_maxNconstituents = 1_pInt
endif
 ...
 enddo
  ...
  do while (trim(line) /= IO_EOF)                                                                    ! read through sections of material part
   ...
   if (section > 0_pInt) then
     ...
     select case(tag)
       case ('(constituent)')
         microstructure_phase(1_pInt, section) =    IO_intValue(line,chunkPos,3_pInt)
     end select
   endif
 enddo

 forall (i = 1_pInt:EBSD_Ngrains) 
   microstructure_crystallite(i) = 1_pInt
   microstructure_texture(1, i) = i
 end forall
 microstructure_fraction = 1.0_pReal
endif 
...
if (EBSD_Ngrains == 0_pInt ) then ! added by zhm
 ...
 texture_Ngauss = 1_pInt
 texture_maxNgauss = 1_pInt
 texture_maxNfiber = 0_pInt
endif
